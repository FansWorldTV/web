...
class SystemPartnerConfiguration extends \Kaltura\Client\ObjectBase
{
	...
	public function __construct(\SimpleXMLElement $xml = null)
	{
		...
		if(empty($xml->permissions))
			...
			$this->permissions = \Kaltura\Client\Client::unmarshalItem($xml->permissions);
		...
		if(empty($xml->limits))
			...
			$this->limits = \Kaltura\Client\Client::unmarshalItem($xml->limits);
		...
		if(!empty($xml->autoModerateEntryFilter))
			$this->autoModerateEntryFilter = \Kaltura\Client\Client::unmarshalItem($xml->autoModerateEntryFilter);
