...
class GenericDistributionProfile extends DistributionProfile
{
	...
	public function __construct(\SimpleXMLElement $xml = null)
	{
		...
		if(!empty($xml->submitAction))
			$this->submitAction = \Kaltura\Client\Client::unmarshalItem($xml->submitAction);
		if(!empty($xml->updateAction))
			$this->updateAction = \Kaltura\Client\Client::unmarshalItem($xml->updateAction);
		if(!empty($xml->deleteAction))
			$this->deleteAction = \Kaltura\Client\Client::unmarshalItem($xml->deleteAction);
		if(!empty($xml->fetchReportAction))
			$this->fetchReportAction = \Kaltura\Client\Client::unmarshalItem($xml->fetchReportAction);
