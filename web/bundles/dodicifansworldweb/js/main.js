...
var site = {
    ...
    init: function() {
        ...
        // Delegate boton de idolship

        var idolizeButton = function(element) {
            $(element).fwIdolship({
        ...
        $(document).ready(function () {
            // Elementos estáticos
            idolizeButton($("[data-idolship-add]:not('[data-override]')"));
            return;
        }).on('DOMNodeInserted', function(event) {
            // Elementos dinámicos
            $(event.target).find("[data-idolship-add]:not('[data-override]')").each(function() {
                idolizeButton(this);
                return;
            });
        });
